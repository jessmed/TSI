; EJERCICIO 5
;
; 1.Crear acción 'Investigar' para llevar a cabo investigaciones
;
; 2.Las investigaciones se realizan en el edificio 'Bahia de Ingenieria'
; 3.Las investigaciones requieren recursos
; 4.Modificamos reclutar para que se necesite la condicion de tener la investigación
;   'Impulsar Segador' para poder reclutar Segadores
...
(define (domain ejercicio_5)
	...
	(:predicates
		...
		; ### NUEVO ###
		; Para comprobar de que tipo es una investigación
		(esInvestigacion ?inves - investigacion ?tInves - tipoInvestigacion )

		; Para saber si una investigación se ha llevado a cabo
		(hayInvestigacion ?invest - investigacion)
	)
	...
	(:action reclutar
		...
		:precondition
			(and
				...
				; y que la investigacion de ImpulsorSegador se haya llevado a cabo
				(imply
					...
					(and
						...
						(hayRecurso Gas)
						; ## NUEVO ##
						(exists (?t - investigacion) (and (hayInvestigacion ?t) (esInvestigacion ?t ImpulsorSegador)) )
	...
	; ## NUEVO ##
	; Accion investigar: sirve para llevar a cabo investigaciones
	; Parámetros: Edificio, Investigación
	(:action investigar
		:parameters (?edif - edificio ?inves - investigacion )

		:precondition
			(and
				; Debe existir el edificio bahia de ingeniería
				(esEdificio ?edif BahiaIngenieria)

				; La bahía debe estar en una localización
				(exists (?l - localizacion) (entidadEnLocalizacion ?edif ?l) )

				; La investigación a realizar no se ha hecho aún
				(not (hayInvestigacion ?inves))

				; Para cada uno de los recurso necesario para la investiación
				; comprobamos que tenemos reservas
				(forall (?r - tipoRecurso)
		...
		:effect
			(and
				; Como resultado notamos que la investigación se ha realizado
				(hayInvestigacion ?inves)
			)
	)
